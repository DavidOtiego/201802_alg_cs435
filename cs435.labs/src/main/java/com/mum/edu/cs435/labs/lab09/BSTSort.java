...
public class BSTSort extends Sorter {
	/** The tree root. */
	private Node root;
	private Queue<Integer> sortedTreeNodes = new ArrayDeque<Integer>();
			...
			while (!inserted) {
				if (x.compareTo(n.element) < 0) {
					...
					throw new RuntimeException(
							"The element " + x + " is already in the tree; " + "duplicates are not allowed");
				}
	...
	public int[] sort(int[] arr) {
		...
		if (!sortedTreeNodes.isEmpty()) {
			anArray = new int[sortedTreeNodes.size()];

			for (int i = 0; i < anArray.length; i++) {
				anArray[i] = sortedTreeNodes.remove();
			}
		}
